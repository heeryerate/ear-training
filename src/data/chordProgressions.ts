...
import { transposeNote, getSemitonesFromC } from './keyCenters';
...
export const transposeProgression = (progressionKey: string, targetKey: string): ChordProgression => {
  const baseProgression = baseProgressions[progressionKey];
  if (!baseProgression) throw new Error(`Unknown progression: ${progressionKey}`);
